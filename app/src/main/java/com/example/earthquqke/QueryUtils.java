...
public class QueryUtils {
    ...
    public static ArrayList<Eathquakehelperclass> extractEarthquakes()
    {
        ArrayList<Eathquakehelperclass> earthquakes=new ArrayList<>();
        try {
            JSONObject baseJsonResponse=new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray earthquakeArray=baseJsonResponse.getJSONArray("features");
            for(int i=0;i<earthquakeArray.length();i++)
            {
               JSONObject obj=earthquakeArray.getJSONObject(i);
    ...
    public static ArrayList<Eathquakehelperclass> FetchEarthquakeData(String stringurl)
    {
        URL url=CreateUrl(stringurl);
        String jsonresponse="";
        try {
             jsonresponse=makeHttpRequest(url);
            Log.d("sattu",jsonresponse);
        } catch (IOException e) {
            Log.e("sattu", "Problem making the HTTP request.", e);
        }
        ArrayList<Eathquakehelperclass> earthquakes=extractFeatureFromJson(jsonresponse);
        return earthquakes;
    }
    private static ArrayList<Eathquakehelperclass>extractFeatureFromJson(String earthquakeJSON)
    {
    ...
    private static URL CreateUrl(String stringurl)
    {
        ...
        try {
            ...
            Log.d("sattu", url.toString());
        } catch (MalformedURLException e) {
            e.printStackTrace();
            Log.e("sattu", "Problem building the URL ", e);
        }
        return url;
    }
    private static String makeHttpRequest(URL url) throws IOException {
        ...
        try {
            ...
            if (urlConnection.getResponseCode() == 200) {
                ...
                Log.e("sattu", "Error response code: " + urlConnection.getResponseCode());
            }
        } catch (IOException e) {
            Log.e("sattu", "Problem retrieving the earthquake JSON results.", e);
        } finally {
